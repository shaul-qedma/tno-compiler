"""Sweep 2: validate the `ansatz_depth ≈ ceil(steps/2)` rule and push
into larger n / longer evolution to find where it breaks.

Augments `compression_grid.csv` with:
- finer `ansatz_depth` grid (3, 5, 6, 10 values missing from sweep 1)
- longer evolutions (steps = 12, 16)
- larger n (up to 14 — statevector sim still fast there)
- skips configs already in the existing CSV

Output: docs/data/compression_grid2.csv
"""
...
prior_path = base / "docs" / "data" / "compression_grid.csv"
...
# Skip set from prior CSV
seen = set()
if prior_path.exists():
    with open(prior_path) as f:
        for r in csv.DictReader(f):
            key = (int(r['n']), float(r['g']), int(r['steps']),
                   int(r['ansatz_depth']))
            seen.add(key)
    print(f"Loaded {len(seen)} prior configs to skip.", flush=True)
...
print(f"{len(configs)} new configs to run.", flush=True)
